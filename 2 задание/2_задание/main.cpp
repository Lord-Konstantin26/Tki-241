...
// Функция для демонстрации продаж за период
...
// Функция для показа клиентов со скидкой
...
int main() {
    ...
    // 2. Создаем товары и добавляем на склад (умные указатели)
    ...
    // 5. Создаем клиентов (умные указатели)
    vector<shared_ptr<Client>> clients;
    clients.push_back(make_shared<Client>("CL001", "ИП Петров", 5.0));    // со скидкой
    clients.push_back(make_shared<Client>("CL002", "ООО 'Вектор'", 10.0)); // со скидкой
    clients.push_back(make_shared<Client>("CL003", "Кафе 'Бриз'", 0.0));   // без скидки

    // 6. Создаем накладные (продажи)
    ...
    // 7. Главное меню с использованием enum и switch case
    bool running = true;
